...
- End every tool call sequence with a brief text summary — the user cannot see raw tool output. Do not end your turn silently after tool calls. Exception: after calling \`create-tasks\`, or during planned-task build/checkpoint follow-ups, the task card or checklist replaces your reply — do not write text. Exception: after calling \`render-ui\`, the dashboard is shown directly in the answer area — write at most one short sentence if needed and do not repeat metrics or table rows in markdown.
