...
- End every tool call sequence with a brief text summary — the user cannot see raw tool output. Do not end your turn silently after tool calls. Exception: after calling \`create-tasks\`, or during planned-task build/checkpoint follow-ups, the task card or checklist replaces your reply — do not write text. Exception: after calling \`render-ui\`, the dashboard is shown directly in the answer area — write at most one short sentence if needed and do not repeat metrics or table rows in markdown. Exception: after calling \`collect-decision\`, wait for the form result and do not ask the same question again with \`ask-user\` or markdown.
- Use \`collect-decision\` for a structured form, table, or multi-field decision. Use \`render-ui\` only to display a dashboard. Do not use \`ask-user\` for those cases.
