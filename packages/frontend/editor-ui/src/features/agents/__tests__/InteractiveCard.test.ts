...
import { APPROVAL_TOOL_NAME, JSON_RENDER_INTERACTION_TOOL_NAME, WAIT_TOOL_NAME } from '@n8n/api-types';
	...
	spec: {
		...
		elements: {
			...
			form: {
				type: 'DynamicForm',
				props: {
					fields: [{ type: 'select', key: 'next_action', label: 'Next action', value: { options: [{ value: 'go', label: 'Go' }] } }],
				},
			},
			submit: { type: 'Button', props: { label: 'Submit' }, on: { press: { action: 'form.submit' } } },
			cancel: { type: 'Button', props: { label: 'Cancel' }, on: { press: { action: 'form.cancel' } } },
		},
	},
};
