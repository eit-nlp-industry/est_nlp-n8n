...
export function findTailSteerableInteractive(
	...
	return getMessageInteractives(tail).find(
		(payload) =>
			...
			payload.toolName !== WAIT_TOOL_NAME &&
			payload.toolName !== JSON_RENDER_INTERACTION_TOOL_NAME,
	);
}
...
function parseJsonRenderInteractionInput(value: unknown): JsonRenderInteractionInput | undefined {
	...
	return {
		...
		...(typeof value.message === 'string' &&
			value.message.length > 0 && { message: value.message }),
	};
}
...
export function rebuildInteractiveFromHistory(tc: ToolCall): InteractivePayload | undefined {
	...
	const jsonRenderInput =
		parseJsonRenderInteractionInput(tc.suspendPayload) ?? parseJsonRenderInteractionInput(tc.input);
	if (jsonRenderInput) {
		const resolved = tc.output !== undefined;
