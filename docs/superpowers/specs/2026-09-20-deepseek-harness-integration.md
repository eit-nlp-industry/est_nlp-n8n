...
One n8n DeepSeek Harness agent owns one isolated Harness profile. A profile has one Harness home directory (`DSH_HOME`). The profile holds Harness credentials, settings, plugins, presets, and session data.
...
## Current Delivery: Entry Page

The first delivery is frontend-only. It does not create, store, run, or configure a Harness agent.

Implemented behavior:

- Add a `DeepSeek Harness` preview tab after `Agents`.
- Add routes for Overview and project pages.
- Show the Agent-style empty state with an active `Create DeepSeek Harness agent` action.
- Keep the action as a no-op until creation is implemented.
- Explain that creation is coming soon.
